...
def eda_pairplot(table: astropy.table.Table, 
                columns: dict, 
                save_dir: str,
                pair_plots: list = None,
                title: str = None, 
    ...
    - pair_plots: A list of tuples specifying pairs of columns to plot against each other (e.g., [('sfr', 'mass'), ('sfr_UV', 'mass')]).
    ...
    n_rows = int(np.ceil((len(pair_plots)) / n_cols))  
    ...
    for i, col in enumerate(pair_plots):
        r = i // n_cols
        c = i % n_cols

        ax = fig.add_subplot(gs[r, c])

        data_x = pd.to_numeric(df[col[0]], errors="coerce")
        data_y = pd.to_numeric(df[col[1]], errors="coerce")

        data_x = data_x.replace([np.inf, -np.inf], np.nan).dropna()
        data_y = data_y.replace([np.inf, -np.inf], np.nan).dropna()
        ...
        ax.set_title(f"{columns[col[0]]} vs {columns[col[1]]}")
        ax.set_xlabel(columns[col[0]])
        ax.set_ylabel(columns[col[1]])


        # ---- Save individual plot ----
        indiv_fig, indiv_ax = plt.subplots(figsize=(6, 4))
        sns.scatterplot(x=data_x, y=data_y, ax=indiv_ax)
        indiv_ax.set_title(f"{columns[col[0]]} vs {columns[col[1]]}")
        indiv_ax.set_xlabel(columns[col[0]])
        indiv_ax.set_ylabel(columns[col[1]])

        indiv_file = f"{save_dir}/{col[0]}_vs_{col[1]}.png"
